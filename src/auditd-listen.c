...
static int init_tls_server_context(struct daemon_conf *config)
{
	...
	SSL_CTX_set_min_proto_version(tls_server_ctx, TLS1_3_VERSION);
	SSL_CTX_set_max_early_data(tls_server_ctx, 0);
	SSL_CTX_set_num_tickets(tls_server_ctx, 0);
...
#ifdef HAVE_TLS
	/* TLS config changes require a restart; free the new config's
	 * copies since they won't be transferred to oconf */
